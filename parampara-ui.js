...
// Builds a route object matching kmCurrentRoute shape, then
// reuses ALL existing infrastructure: kmDrawRoute, kmShowPlaybar,
// kmFocusStop, kmRenderStopList, kmStepRoute, kmPlayRoute.
// ═══════════════════════════════════════════════════════════
...
// Register pilgrimage colour so existing legend/draw code works
KM_ROUTE_COLORS['Carnatic Pilgrimage'] = { line:'#C4A044', pin:'#C4A044', bg:'rgba(196,160,68,0.15)' };
...
function kmInitPilgrimCities() {
  ...
  KM_PILGRIM_CITIES.forEach(c => {
    ...
    item.onclick = () => {
      ...
      // Immediately build and launch the pilgrimage — no button needed
      kmBuildPilgrimage(c);
    };
    list.appendChild(item);
  });
...
function kmPilgrimScore(t) {
  ...
  const city = (t.location_city || '').toLowerCase();
  if (t.pancha_bhuta_stala) score += 30;
  if (t.divya_desam_number) score += 20;
  const compIds = new Set((kmData.junctions||[]).filter(j=>j.kshetra_id===t.id).map(j=>j.composition_id));
  const nc = compIds.size;
  if (nc >= 10) score += 25; else if (nc >= 5) score += 18; else if (nc >= 3) score += 12; else if (nc >= 1) score += 8;
  // Trinity composers by known IDs
  const trinityIds = ['cd478bac-fc19-4fc7-b30a-4045a97b9806','b6dbb72e-0cc3-43e2-a997-a291778a449e','f4cd87e3-ea8e-4850-a7b6-62db009a085c'];
  const hasT = (kmData.compositions||[]).filter(c=>compIds.has(c.id)).some(c=>trinityIds.includes(c.composer_id));
  if (hasT) score += 10;
  const clusters = ['tiruvarur','chidambaram','thanjavur','srirangam','tirupati','madurai','rameswaram','kanchipuram','tiruvannamalai'];
  if (clusters.some(cl => name.includes(cl) || city.includes(cl))) score += 15;
  return score;
}
...
function kmBuildPilgrimage(city) {
  ...
  // Score → top 80 → nearest-neighbour route
  const scored = temples.map(t=>({...t,_score:kmPilgrimScore(t)})).sort((a,b)=>b._score-a._score).slice(0,80);
  let cur = { lat:city.lat, lng:city.lng };
  const unvisited = [...scored];
  const orderedTemples = [];
  while (unvisited.length) {
    let ni=-1, nd=Infinity;
    unvisited.forEach((t,i)=>{ const d=kmDistKm(cur.lat,cur.lng,t.latitude,t.longitude); if(d<nd){nd=d;ni=i;} });
    orderedTemples.push({...unvisited[ni], _distFromPrev:Math.round(nd)});
    cur={lat:unvisited[ni].latitude, lng:unvisited[ni].longitude};
    unvisited.splice(ni,1);
  }

  // Build a route object in the EXACT shape kmCurrentRoute expects
  const routeObj = {
    name: 'Carnatic Pilgrimage',
    description: `Optimised from ${city.name}`,
    circa: '',
    stops: orderedTemples.map((t, i) => {
      // Find a representative composition for this temple
      const compIds = new Set((kmData.junctions||[]).filter(j=>j.kshetra_id===t.id).map(j=>j.composition_id));
      const comp = (kmData.compositions||[]).find(c=>compIds.has(c.id)) || null;
      return {
        sequence_number: i + 1,
        travel_note: `${t._distFromPrev} km from previous stop`,
        kshetra: {
          id: t.id,
  ...
  // Clear any existing route, then hand off to existing infrastructure
  kmClearRoute();
  kmCurrentRoute = routeObj;
  kmPlayStep = 0;

  // Show route panel (reuse existing panel)
  document.getElementById('km-route-info').textContent = `${orderedTemples.length} temples · starting from ${city.name}`;
  document.getElementById('km-route-panel').style.display = 'flex';

  kmRenderStopList(routeObj.stops, -1);
...
// City list initialised inside kmInit() after data loads
