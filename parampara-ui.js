...
// Nearest-neighbour routing from a chosen starting city
// ═══════════════════════════════════════════════════════════
...
let kmPilgrimRoute = [];
let kmPilgrimMarkers = [];
let kmPilgrimPolyline = null;
let kmPilgrimStep = 0;

function kmInitPilgrimCities() {
  const list = document.getElementById('km-pilgrim-city-list');
  if (!list || list.children.length > 0) return;
  KM_PILGRIM_CITIES.forEach(c => {
    const item = document.createElement('div');
    item.className = 'km-dropdown-item';
    item.textContent = c.name;
    item.dataset.val = c.name;
    item.onclick = () => {
      document.getElementById('km-pilgrim-city-btn').textContent = c.name;
      document.getElementById('km-pilgrim-city-btn').dataset.val = c.name;
      document.querySelectorAll('#km-pilgrim-city-list .km-dropdown-item').forEach(el => el.classList.remove('selected'));
      item.classList.add('selected');
      // Close dropdown
      document.getElementById('km-pilgrim-city-dd').classList.remove('open');
      document.getElementById('km-pilgrim-go-btn').style.display = 'block';
    };
    list.appendChild(item);
  });
}

function kmDistKm(lat1, lng1, lat2, lng2) {
  const R = 6371;
...
function kmPilgrimScore(t) {
  let score = 0;
  const name = (t.temple_name || '').toLowerCase();
  // Pancha Bhuta Stala
  if (t.is_pancha_bhuta) score += 30;
  // Divya Desam
  if (t.is_divya_desam) score += 20;
  // Kriti count via junctions
  const compIds = new Set(
    (kmData.junctions || []).filter(j => j.kshetra_id === t.id).map(j => j.composition_id)
  );
  const nc = compIds.size;
  if (nc >= 10) score += 25;
  else if (nc >= 5) score += 18;
  else if (nc >= 3) score += 12;
  else if (nc >= 1) score += 8;
  // Trinity composers (Dikshitar, Tyagaraja, Syama Sastri — check by name)
  const trinityNames = ['dikshitar','tyagaraja','syama'];
  const composerNames = (kmData.compositions || [])
    .filter(c => compIds.has(c.id))
    .map(c => (c.composer_name || c.composer || '').toLowerCase());
  if (composerNames.some(n => trinityNames.some(t => n.includes(t)))) score += 10;
  // Major pilgrimage clusters
  const clusters = ['tiruvarur','chidambaram','thanjavur','srirangam','tirupati','madurai','rameswaram','kanchipuram','tiruvannamalai'];
  if (clusters.some(c => name.includes(c) || (t.location_city||'').toLowerCase().includes(c))) score += 15;
  return score;
}

function kmBuildPilgrimage() {
  const cityName = document.getElementById('km-pilgrim-city-btn').dataset.val;
  if (!cityName) return;
  const city = KM_PILGRIM_CITIES.find(c => c.name === cityName);
  if (!city) return;

  // Get all temples with coordinates
  const temples = (kmData.temples || []).filter(t => t.latitude && t.longitude);
  if (!temples.length) { alert('Temple data not loaded yet. Please wait a moment and try again.'); return; }

  // Score and take top 80
  const scored = temples.map(t => ({ ...t, _score: kmPilgrimScore(t) }))
    .sort((a,b) => b._score - a._score)
    .slice(0, 80);

  // Nearest-neighbour from city
  let current = { lat: city.lat, lng: city.lng };
  const unvisited = [...scored];
  const route = [];

  while (unvisited.length) {
    let nearest = null, nearestIdx = -1, nearestDist = Infinity;
    unvisited.forEach((t, i) => {
      const d = kmDistKm(current.lat, current.lng, t.latitude, t.longitude);
      if (d < nearestDist) { nearestDist = d; nearest = t; nearestIdx = i; }
    });
    route.push({ ...nearest, _distFromPrev: Math.round(nearestDist) });
    current = { lat: nearest.latitude, lng: nearest.longitude };
    unvisited.splice(nearestIdx, 1);
  }

  kmPilgrimRoute = route;
  kmPilgrimStep = 0;
  kmDrawPilgrimRoute(city);
  kmRenderPilgrimStops(route);

  document.getElementById('km-pilgrim-info').textContent =
    `${route.length} temples · starting from ${cityName} · nearest-neighbour routing`;
  document.getElementById('km-pilgrim-panel').style.display = 'block';
}

function kmDrawPilgrimRoute(city) {
  // Clear previous
  kmClearPilgrimage(true);

  const latlngs = [
    [city.lat, city.lng],
    ...kmPilgrimRoute.map(t => [t.latitude, t.longitude])
  ];

  kmPilgrimPolyline = L.polyline(latlngs, {
    color: '#C4A044', weight: 2, opacity: 0.7, dashArray: '5,6'
  }).addTo(kmMap);

  // Numbered markers
  kmPilgrimRoute.forEach((t, i) => {
    const icon = L.divIcon({
      className: '',
      html: `<div style="background:#C4A044;color:#fff;border-radius:50%;width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;font-size:10px;font-weight:700;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.3);">${i+1}</div>`,
      iconSize: [20, 20], iconAnchor: [10, 10]
    });
    const marker = L.marker([t.latitude, t.longitude], { icon })
      .addTo(kmMap)
      .on('click', () => kmPilgrimFocusStop(i));
    kmPilgrimMarkers.push(marker);
  });

  // Fit map to route
  kmMap.fitBounds(kmPilgrimPolyline.getBounds(), { padding: [40, 40] });
}

function kmRenderPilgrimStops(route) {
  const el = document.getElementById('km-pilgrim-stops');
  el.innerHTML = route.map((t, i) => {
    const compIds = new Set(
      (kmData.junctions || []).filter(j => j.kshetra_id === t.id).map(j => j.composition_id)
    );
    const kritis = (kmData.compositions || [])
      .filter(c => compIds.has(c.id)).slice(0, 2).map(k => k.title).join(', ');
    return `<div class="km-stop-item" id="km-ps-${i}" onclick="kmPilgrimFocusStop(${i})" style="padding:8px 18px;cursor:pointer;border-bottom:1px solid var(--bo);transition:background 0.15s;">
      <div style="display:flex;align-items:flex-start;gap:8px;">
        <span style="min-width:20px;height:20px;background:#C4A044;color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;font-size:10px;font-weight:700;flex-shrink:0;">${i+1}</span>
        <div>
          <div style="font-family:'EB Garamond',serif;font-size:13px;color:var(--tx);line-height:1.3;">${t.temple_name || ''}</div>
          <div style="font-family:'EB Garamond',serif;font-size:11px;color:var(--txl);margin-top:1px;">${t.location_city || ''}${t.location_state ? ', '+t.location_state : ''} · ${t._distFromPrev} km</div>
          ${kritis ? `<div style="font-family:'EB Garamond',serif;font-size:11px;color:var(--txm);font-style:italic;margin-top:2px;">${kritis}</div>` : ''}
        </div>
      </div>
    </div>`;
  }).join('');
...
function kmPilgrimFocusStop(i) {
  kmPilgrimStep = i;
  const t = kmPilgrimRoute[i];
  if (!t) return;
  kmMap.setView([t.latitude, t.longitude], 10, { animate: true });
  // Highlight stop in list
  document.querySelectorAll('#km-pilgrim-stops .km-stop-item').forEach((el, idx) => {
    el.style.background = idx === i ? 'rgba(196,160,68,0.12)' : '';
  });
  // Scroll stop into view
  const stopEl = document.getElementById('km-ps-' + i);
  if (stopEl) stopEl.scrollIntoView({ behavior:'smooth', block:'nearest' });
  // Open temple popup if marker exists
  if (kmPilgrimMarkers[i]) kmPilgrimMarkers[i].openPopup();
}

function kmClearPilgrimage(silent) {
  if (kmPilgrimPolyline) { kmMap.removeLayer(kmPilgrimPolyline); kmPilgrimPolyline = null; }
  kmPilgrimMarkers.forEach(m => kmMap.removeLayer(m));
  kmPilgrimMarkers = [];
  kmPilgrimRoute = [];
  if (!silent) {
    document.getElementById('km-pilgrim-panel').style.display = 'none';
    document.getElementById('km-pilgrim-city-btn').textContent = '— Select city —';
    document.getElementById('km-pilgrim-city-btn').dataset.val = '';
    document.getElementById('km-pilgrim-go-btn').style.display = 'none';
    document.querySelectorAll('#km-pilgrim-city-list .km-dropdown-item').forEach(el => el.classList.remove('selected'));
  }
}

// City list is initialised inside kmInit() after data loads
