...
function showProfScreen(){showSc('ss');document.getElementById('stats-loaded').style.display='none';document.getElementById('profile-screen').style.display='flex';}
function showProfSearch(){document.getElementById('prof-results').innerHTML='';document.getElementById('prof-input').value='';document.getElementById('prof-detail').style.display='none';document.getElementById('prof-results').style.display='none';document.getElementById('prof-search-box').style.display='flex';}
async function profSearch(){const q=(document.getElementById('prof-input').value||'').trim();if(!q)return;document.getElementById('prof-results').innerHTML='<div style="font-family:\'EB Garamond\',serif;font-style:italic;color:var(--txl);padding:12px 0;">Consulting the heritage ledger…</div>';document.getElementById('prof-results').style.display='block';const res=await db.from('profiles').select('id,full_name,primary_role,current_country,instrument_aliases,is_deceased').ilike('full_name','%'+q+'%').limit(20);if(!res.data||!res.data.length){document.getElementById('prof-results').innerHTML='<div style="padding:16px;font-family:\'EB Garamond\',serif;font-style:italic;color:var(--txl);">No one found matching "'+q+'". <button onclick="window.location.href=\'mailto:join@parampara.io?subject=Join Parampara\'" style="font-family:\'Cinzel\',serif;font-size:8px;letter-spacing:0.2em;color:var(--g);background:none;border:1px solid var(--bo);padding:6px 16px;border-radius:2px;cursor:pointer;margin-left:8px;">REQUEST TO JOIN</button></div>';return;}if(res.data.length===1){profShowDetail(res.data[0]);return;}let html='<div style="font-family:\'Cinzel\',serif;font-size:9px;letter-spacing:0.2em;color:var(--txl);margin-bottom:12px;">'+res.data.length+' MATCHES</div>';res.data.forEach(p=>{let ins='';if(p.instrument_aliases){let a=p.instrument_aliases;if(typeof a==='string'){try{a=JSON.parse(a);}catch{}}if(Array.isArray(a))ins=a.filter(x=>x&&x!=='Vocal').slice(0,2).join(', ');}html+='<div onclick="profShowDetail('+JSON.stringify(p).replace(/"/g,'&quot;')+')" style="background:var(--s);border:1px solid var(--bo2);border-radius:3px;padding:14px 18px;margin-bottom:8px;cursor:pointer;"><div style="font-family:\'Cormorant Garamond\',serif;font-size:19px;font-weight:600;color:var(--br);">'+p.full_name+'</div><div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:13px;color:var(--txl);">'+(p.primary_role||'')+(ins?' · '+ins:'')+(p.current_country?' · '+p.current_country:'')+'</div></div>';});document.getElementById('prof-results').innerHTML=html;}
async function profShowDetail(profile){if(typeof profile==='string')profile=JSON.parse(profile);document.getElementById('prof-search-box').style.display='none';const dv=document.getElementById('prof-detail');dv.style.display='flex';dv.innerHTML='<div style="font-family:\'EB Garamond\',serif;font-style:italic;color:var(--txl);">Loading…</div>';const{data:links}=await db.from('guru_shishya_lineage').select('shishya_id,guru_id,discipline,is_verified').or('shishya_id.eq.'+profile.id+',guru_id.eq.'+profile.id).neq('discipline','Heritage');const ids=new Set();(links||[]).forEach(l=>{ids.add(l.guru_id);ids.add(l.shishya_id);});ids.delete(profile.id);const{data:rel}=ids.size>0?await db.from('profiles').select('id,full_name,primary_role,is_deceased').in('id',[...ids]):{data:[]};const pm={};(rel||[]).forEach(p=>{pm[p.id]=p;});pm[profile.id]=profile;const gurus=(links||[]).filter(l=>l.shishya_id===profile.id).map(l=>({...pm[l.guru_id],discipline:l.discipline})).filter(g=>g&&g.full_name);const shishyas=(links||[]).filter(l=>l.guru_id===profile.id).map(l=>({...pm[l.shishya_id],discipline:l.discipline})).filter(s=>s&&s.full_name);let ins='';if(profile.instrument_aliases){let a=profile.instrument_aliases;if(typeof a==='string'){try{a=JSON.parse(a);}catch{}}if(Array.isArray(a))ins=a.filter(x=>x&&x!=='Vocal').slice(0,2).join(', ');}dv.innerHTML=`<button onclick="showProfSearch()" style="font-family:'Cinzel',serif;font-size:8px;letter-spacing:0.2em;color:var(--txl);background:none;border:1px solid var(--bo);padding:8px 20px;border-radius:2px;cursor:pointer;margin-bottom:20px;align-self:flex-start;">← Search again</button><div style="background:var(--s);border:1px solid var(--bo2);border-radius:4px;padding:24px 28px;margin-bottom:20px;position:relative;overflow:hidden;"><div style="position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--gl),var(--g));"></div><div style="font-family:'Cinzel',serif;font-size:8px;letter-spacing:0.2em;color:var(--gl);margin-bottom:10px;">✦ HERITAGE LEDGER</div><div style="font-family:'Cormorant Garamond',serif;font-size:30px;font-weight:600;color:var(--br);margin-bottom:4px;">${profile.full_name||''}</div><div style="font-family:'EB Garamond',serif;font-style:italic;font-size:15px;color:var(--txl);margin-bottom:14px;">${profile.primary_role||''}${ins?' · '+ins:''}${profile.current_country?' · '+profile.current_country:''}</div>${gurus.length?`<div style="font-family:'Cinzel',serif;font-size:8px;letter-spacing:0.2em;color:var(--txl);margin-bottom:6px;">GURUS</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;">${gurus.map(g=>`<span style="font-family:'EB Garamond',serif;font-size:13px;color:var(--br);background:rgba(107,63,26,0.06);border:1px solid rgba(107,63,26,0.15);padding:3px 10px;border-radius:2px;">${g.is_deceased?'✦ ':''}${g.full_name}</span>`).join('')}</div>`:''} ${shishyas.length?`<div style="font-family:'Cinzel',serif;font-size:8px;letter-spacing:0.2em;color:var(--txl);margin-bottom:6px;">SHISHYAS</div><div style="display:flex;flex-wrap:wrap;gap:6px;">${shishyas.map(s=>`<span style="font-family:'EB Garamond',serif;font-size:13px;color:var(--br);background:rgba(107,63,26,0.06);border:1px solid rgba(107,63,26,0.15);padding:3px 10px;border-radius:2px;">${s.full_name}</span>`).join('')}</div>`:''}<div style="display:flex;gap:20px;margin-top:14px;padding-top:14px;border-top:1px solid var(--bo2);"><div style="text-align:center;"><div style="font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:300;color:var(--g);">${gurus.length}</div><div style="font-family:'Cinzel',serif;font-size:7px;letter-spacing:0.2em;color:var(--txl);">GURUS</div></div><div style="text-align:center;"><div style="font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:300;color:var(--g);">${shishyas.length}</div><div style="font-family:'Cinzel',serif;font-size:7px;letter-spacing:0.2em;color:var(--txl);">SHISHYAS</div></div></div></div>`;setTimeout(()=>profDrawLineage(profile,links||[],pm),100);}
function profDrawLineage(profile,links,pm){const wrap=document.getElementById('prof-detail');const svgWrap=document.createElement('div');svgWrap.style.cssText='background:var(--s);border:1px solid var(--bo2);border-radius:4px;overflow:hidden;';svgWrap.innerHTML='<svg id="prof-svg" style="width:100%;height:460px;cursor:grab;"></svg>';wrap.appendChild(svgWrap);const svg=d3.select('#prof-svg');const W=document.getElementById('prof-svg').clientWidth||800,H=460;const g=svg.append('g');svg.call(d3.zoom().scaleExtent([0.3,3]).on('zoom',e=>g.attr('transform',e.transform)));const nodes=[],nodeMap={};const addNode=(id,isCenter)=>{if(nodeMap[id])return;const p=pm[id];if(!p)return;const n={id,name:p.full_name||'',role:p.primary_role||'',isCenter,isDeceased:p.is_deceased};nodes.push(n);nodeMap[id]=n;};addNode(profile.id,true);links.forEach(l=>{addNode(l.guru_id,false);addNode(l.shishya_id,false);});const guruIds=new Set(links.filter(l=>l.shishya_id===profile.id).map(l=>l.guru_id));const shishyaIds=new Set(links.filter(l=>l.guru_id===profile.id).map(l=>l.shishya_id));const cx=W/2,cy=H/2;nodeMap[profile.id].x=cx;nodeMap[profile.id].y=cy;const gn=nodes.filter(n=>guruIds.has(n.id)),sn=nodes.filter(n=>shishyaIds.has(n.id));gn.forEach((n,i)=>{const a=(-Math.PI/2)+(i-(gn.length-1)/2)*(gn.length>1?(Math.PI*0.7/(gn.length-1)):0);n.x=cx+190*Math.cos(a);n.y=cy-150+80*Math.sin(a);});sn.forEach((n,i)=>{const a=(Math.PI/2)+(i-(sn.length-1)/2)*(sn.length>1?(Math.PI*0.7/(sn.length-1)):0);n.x=cx+190*Math.cos(a);n.y=cy+150+80*Math.sin(a);});links.forEach(l=>{const s=nodeMap[l.guru_id],t=nodeMap[l.shishya_id];if(!s||!t)return;g.append('line').attr('x1',s.x).attr('y1',s.y).attr('x2',t.x).attr('y2',t.y).attr('stroke',l.is_verified?'#C4A044':'#B0A080').attr('stroke-width',1.5).attr('stroke-dasharray',l.is_verified?'none':'5,4').attr('opacity',0.7);const mx=(s.x+t.x)/2,my=(s.y+t.y)/2;g.append('text').attr('x',mx).attr('y',my-4).attr('text-anchor','middle').attr('font-family','EB Garamond,serif').attr('font-style','italic').attr('font-size','11px').attr('fill','#8A7040').text(l.discipline||'');});nodes.forEach(n=>{const ng=g.append('g').attr('transform',`translate(${n.x},${n.y})`);if(n.isCenter){ng.append('circle').attr('r',32).attr('fill','#C4A044').attr('opacity',0.15);ng.append('circle').attr('r',22).attr('fill','#7A5C10');ng.append('text').attr('text-anchor','middle').attr('dominant-baseline','middle').attr('font-family','Cormorant Garamond,serif').attr('font-weight','600').attr('font-size','11px').attr('fill','#FDFAF3').text(n.name.split(' ')[0]);ng.append('text').attr('y',34).attr('text-anchor','middle').attr('font-family','Cormorant Garamond,serif').attr('font-size','13px').attr('font-weight','600').attr('fill','#6B3F1A').text(n.name.length>24?n.name.substring(0,22)+'…':n.name);}else{ng.append('circle').attr('r',18).attr('fill',n.isDeceased?'none':'var(--iv)').attr('stroke',n.isDeceased?'#C4A044':'#8AA8C8').attr('stroke-width',1.5).attr('stroke-dasharray',n.isDeceased?'4,3':'none');ng.append('text').attr('text-anchor','middle').attr('dominant-baseline','middle').attr('font-family','EB Garamond,serif').attr('font-size','9px').attr('fill','var(--tx)').text(n.name.split(' ')[0]);ng.append('text').attr('y',26).attr('text-anchor','middle').attr('font-family','Cormorant Garamond,serif').attr('font-size','12px').attr('font-weight','500').attr('fill','#5A4020').text(n.name.length>20?n.name.substring(0,18)+'…':n.name);ng.append('text').attr('y',38).attr('text-anchor','middle').attr('font-family','EB Garamond,serif').attr('font-style','italic').attr('font-size','10px').attr('fill','#8A7040').text(n.role);}});}
...
function kmInitPilgrimCities() {
  ...
  KM_PILGRIM_CITIES.forEach(city => {
    ...
    div.className = 'km-dropdown-item';
...
function kmPilgrimScore(temple) {
  ...
  const junctionCount = (kmData.junctions || []).filter(j => j.kshetra_id === temple.id).length;
  if      (junctionCount >= 10) score += 25;
  else if (junctionCount >= 5)  score += 18;
  else if (junctionCount >= 2)  score += 10;
  else if (junctionCount >= 1)  score += 8;
  const trinityIds = [
    'cd478bac-fc19-4fc7-b30a-4045a97b9806',
    'b6dbb72e-0cc3-43e2-a997-a291778a449e',
    'f4cd87e3-ea8e-4850-a7b6-62db009a085c'
  ];
  const composersHere = (kmData.junctions||[])
    .filter(j => j.kshetra_id === temple.id)
    .map(j => { const c = (kmData.compositions||[]).find(x=>x.id===j.composition_id); return c ? c.composer_id : null; })
    .filter(Boolean);
  if (composersHere.some(id => trinityIds.includes(id))) score += 10;
  const cluster = ['thanjavur','chidambaram','kumbakonam','madurai','tiruchirapalli','tiruvarur'];
  const city = (temple.location_city||'').toLowerCase();
  if (cluster.some(c => city.includes(c))) score += 15;
  return score;
}

function kmBuildPilgrimage(city) {
  // Score & pick top 80 temples
  const scored = kmData.temples
    .filter(t => t.latitude && t.longitude)
    .map(t => ({ t, score: kmPilgrimScore(t) }))
    .sort((a,b) => b.score - a.score)
    .slice(0, 80)
    .map(x => x.t);

  // Nearest-neighbour routing from city
  const ordered = [];
  ...
  while (remaining.length) {
    ...
    remaining.forEach((t,i) => {
      const d = Math.hypot(t.latitude - curLat, t.longitude - curLng);
      if (d < bestDist) { bestDist = d; best = i; }
    });
    const chosen = remaining.splice(best, 1)[0];
    ordered.push(chosen);
    curLat = chosen.latitude; curLng = chosen.longitude;
  }

  // Build stops array matching kmCurrentRoute shape
  const stops = ordered.map((t, i) => {
    ...
    return {
      ...
      travel_note: '',
      kshetra: {
        id: t.id, temple_name: t.temple_name,
        lat: t.latitude, lng: t.longitude,
        latitude: t.latitude, longitude: t.longitude,
        location_city: t.location_city, location_state: t.location_state,
        primary_deity: t.primary_deity, tradition: t.tradition,
        designation_tags: t.designation_tags, significance: t.significance,
        historical_significance: t.historical_significance, image_url: t.image_url
      },
      composition: comp ? { id: comp.id, title: comp.title, raga: comp.raga } : null
    };
  });

  const routeObj = {
    name: 'Carnatic Pilgrimage',
    route_type: 'Carnatic Pilgrimage',
    description: `A personalised Carnatic pilgrimage starting from ${city.name}, visiting ${stops.length} sacred temples.`,
    stops
  };

  kmClearRoute();
  kmCurrentRoute = routeObj;
  kmPlayStep = 0;
  kmDrawRoute(routeObj);
  kmShowPlaybar(routeObj);
  kmFocusStop(0);
  kmRenderStopList(routeObj.stops, 0);
}
...
function kmResetView() {
  ...
  // Reset pilgrim city dropdown
  const cityBtn = document.getElementById('km-city-btn');
  if (cityBtn) cityBtn.textContent = '— Select a city —';
  const cityDD = document.getElementById('km-city-dd');
  if (cityDD) { const b = cityDD.querySelector('.km-dropdown-btn'); if(b) b.classList.remove('open'); const l = cityDD.querySelector('.km-dropdown-list'); if(l) l.classList.remove('open'); }
